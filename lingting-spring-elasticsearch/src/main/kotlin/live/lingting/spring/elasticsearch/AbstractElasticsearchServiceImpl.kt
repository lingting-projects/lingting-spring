...
abstract class AbstractElasticsearchServiceImpl<T : Any> {
    ...
    fun newApi(properties: ElasticsearchProperties, client: ElasticsearchClient, interceptors: List<Interceptor>, polymerizeFacroty: PolymerizeFactory) {
        if (api != null) {
            return
        }
        api = ElasticsearchApi(cls, polymerizeFacroty, { documentId(it) }, properties, interceptors, client)
    }
